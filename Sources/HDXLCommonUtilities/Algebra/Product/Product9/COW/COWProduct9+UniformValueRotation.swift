//
//  COWProduct9+UniformValueRotation.swift
//

import Foundation

// -------------------------------------------------------------------------- //
// MARK: COWProduct9 - UniformValueRotation
// -------------------------------------------------------------------------- //
...
public extension COWProduct9 
  ...
  @inlinable
  func pushedLeftward(byAppending value: A) -> UniformCOWProduct9<A> {
    return UniformCOWProduct9<A>(
      self.b,
      self.c,
      self.d,
      self.e,
  ...
  @inlinable
  func pushedRightward(byPrepending value: A) -> UniformCOWProduct9<A> {
    return UniformCOWProduct9<A>(
      value,
      self.a,
      self.b,
  ...
  @inlinable
  func weaklyPushedLeftward(byAppending value: A?) -> UniformCOWProduct9<A>? {
  ...
  /// When `value` is `nil` the result is just `nil`.
  @inlinable
  func weaklyPushedRightward(byPrepending value: A?) -> UniformCOWProduct9<A>? {
    guard let concreteValue = value else {
      return nil
    }
