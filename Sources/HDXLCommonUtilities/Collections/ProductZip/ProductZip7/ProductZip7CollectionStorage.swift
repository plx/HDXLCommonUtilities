...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Definition
// ------------------------------------------------------------------------ //

@usableFromInline
internal final class ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>
  where
  A:Collection,
  B:Collection,
  C:Collection,
...
// -------------------------------------------------------------------------- //
// MARK: ProductZip7CollectionStorage - Reflection Support
// -------------------------------------------------------------------------- //

internal extension ProductZip7CollectionStorage {
  
  @inlinable
  static var shortTypeName: String {
    get {
      return "ProductZip7CollectionStorage"
    }
  }
...
// -------------------------------------------------------------------------- //
// MARK: ProductZip7CollectionStorage - With Derivation
// -------------------------------------------------------------------------- //
...
internal extension ProductZip7CollectionStorage {
  ...
  @inlinable
  func with(a: A) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
      d,
  ...
  @inlinable
  func with(b: B) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
  ...
  @inlinable
  func with(c: C) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
  ...
  @inlinable
  func with(d: D) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
  ...
  @inlinable
  func with(e: E) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
  ...
  @inlinable
  func with(f: F) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
  ...
  @inlinable
  func with(g: G) -> ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element> {
    return ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>(
      a,
      b,
      c,
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Validatable
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : Validatable {
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Equatable
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : Equatable
  where
  A:Equatable,
  B:Equatable,
  C:Equatable,
  ...
  internal static func ==(
    lhs: ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>,
    rhs: ProductZip7CollectionStorage<A,B,C,D,E,F,G,Position,Element>) -> Bool {
    guard lhs !== rhs else {
      return true
    }
    guard
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Hashable
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : Hashable
  where
  A:Hashable,
  B:Hashable,
  C:Hashable,
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - CustomStringConvertible
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : CustomStringConvertible {
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - CustomDebugStringConvertible
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : CustomDebugStringConvertible {
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Codable
// ------------------------------------------------------------------------ //

extension ProductZip7CollectionStorage : Codable
  where
  A:Codable,
  B:Codable,
  C:Codable,
...
// ------------------------------------------------------------------------ //
// MARK: ProductZip7CollectionStorage - Collection Support
// ------------------------------------------------------------------------ //

internal extension ProductZip7CollectionStorage {
  
  @usableFromInline
  typealias Index = ProductZip7CollectionIndex<Position>
  ...
  func couldHaveCreated(position: Position) -> Bool {
    guard
      ...
      allArgumentsMutuallyEqual(
        ...
        self.g.distanceFromStart(to: position.g)) else {
          return false
      }
    return true
  }
  ...
  func formPosition(after position: inout Position) -> Bool {
    guard
      ...
      self.g.formSubscriptableIndex(after: &position.g) else {
        return false
    }
    return true
  }
...
internal extension ProductZip7CollectionStorage
  ...
  func formPosition(before position: inout Position) -> Bool {
    guard
      ...
      self.g.formSubscriptableIndex(before: &position.g) else {
        return false
    }
    return true
  }
